      ...
      <div style={{ flex: 1 }}>
        ...
        {(brand || shape || modelYear || showOnSale !== null) && (
          <div style={{
            ...
            {brand && <span style={{ backgroundColor: "#E0E0E0", padding: "5px 10px", borderRadius: "15px", fontSize: "14px" }}>Brand: {brand}</span>}
            {shape && <span style={{ backgroundColor: "#E0E0E0", padding: "5px 10px", borderRadius: "15px", fontSize: "14px" }}>Type: {shape}</span>}
            {modelYear && <span style={{ backgroundColor: "#E0E0E0", padding: "5px 10px", borderRadius: "15px", fontSize: "14px" }}>Year: {modelYear}</span>}
            {showOnSale === true && <span style={{ backgroundColor: "#FFD6D6", padding: "5px 10px", borderRadius: "15px", fontSize: "14px", color: "#9E2A2B" }}>Hot Deals Only</span>}
            {showOnSale === false && <span style={{ backgroundColor: "#E0E0E0", padding: "5px 10px", borderRadius: "15px", fontSize: "14px" }}>Regular Prices Only</span>}
          </div>
        )}
          ...
          <div style={{ marginBottom: "40px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: "20px" }}>
              {hotDeals.map((vehicle) => (
                <div key={vehicle.vehicleID} style={{ background: "#FFFFFF", border: "2px solid #9E2A2B", borderRadius: "10px", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
                  <div style={{ height: "150px", backgroundColor: "#f5f5f5", borderBottom: "1px solid #D9D9D9", display: "flex", alignItems: "center", justifyContent: "center", color: "#999", fontSize: "14px" }}>
                    Vehicle Image
                  </div>
                  <div style={{ padding: "15px" }}>
                    <h3 onClick={() => handleClick(vehicle.vehicleID)} style={{ cursor: "pointer", color: "#335C67", margin: "0 0 10px 0", fontSize: "18px" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "15px" }}>
                      <span style={{ fontSize: "16px", fontWeight: "bold", color: "#9E2A2B" }}>${vehicle.price}</span>
                      <button
                        onClick={() => handleAddToCart(vehicle.vehicleID)}
                        disabled={addingToCart[vehicle.vehicleID]}
                        style={{ padding: "8px 15px", backgroundColor: "rgba(224, 159, 62, 0.88)", color: "#000", border: "none", borderRadius: "5px", cursor: "pointer", fontSize: "14px" }}
                      >
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(250px, 1fr))", gap: "20px" }}>
              {regularVehicles.map((vehicle) => (
                <div key={vehicle.vehicleID} style={{ background: "#FFFFFF", border: "1px solid #D9D9D9", borderRadius: "10px", overflow: "hidden", boxShadow: "0 2px 8px rgba(0,0,0,0.1)" }}>
                  <div style={{ height: "150px", backgroundColor: "#f5f5f5", borderBottom: "1px solid #D9D9D9", display: "flex", alignItems: "center", justifyContent: "center", color: "#999", fontSize: "14px" }}>
                    Vehicle Image
                  </div>
                  <div style={{ padding: "15px" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "15px" }}>
                      <span style={{ fontSize: "16px", fontWeight: "bold", color: "#335C67" }}>${vehicle.price}</span>
                      <button
                        onClick={() => handleAddToCart(vehicle.vehicleID)}
                        disabled={addingToCart[vehicle.vehicleID]}
                        style={{ padding: "8px 15px", backgroundColor: "rgba(224, 159, 62, 0.88)", color: "#000", border: "none", borderRadius: "5px", cursor: "pointer", fontSize: "14px" }}
